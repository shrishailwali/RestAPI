            ...
            if data:
                ...
                if payload['tag'] == 'production' and data['task'] == "dlog_process":
                    ...
                    if asset_info:
                        ...
                        print('4', attribute_info)
                        # send_event({"data": attribute_info, "task": "event_log"})
                        # channel.basic_ack(method_frame.delivery_tag)
                        event_receiver(attribute_info)
                    else:
                        print('na lavada lo love story')
                        # channel.basic_ack(method_frame.delivery_tag)
                    conn.commit()
                    conn.close()
                else:
...
def event_receiver(payload):
    ...
    try:
        ...
        # if task and payload:
        if payload:
            # if data['task'] == "event_log" and data['event_code']:
            if payload['task'] == "event_log" and payload['event_code']:
                conn = psycopg2.connect(Dburl)
                cursor = conn.cursor()
                ...
                print(f'''select mm_event_starttime, mm_event_endtime,mm_asset_status, mm_downtime_reason_code, mm_fault_reason_code
                 from mm_eventSequence where fw_eui_id ='{payload['fw_eui_id']}' and fw_tenant_id = {payload['fw_tenant_id']}
                               and sf_asset_id = {payload['sf_asset_id']} and mm_event_date <='{payload['data_received_time']}' order by mm_eventSequence_id desc limit 1;''')
                event_info = cursor.fetchone()
                execute = event_log(event_info,payload, conn)
                print(payload, event_info)
                conn.commit()
                conn.close()
                print('hello doc', execute)
                # if execute:
                    # channel.basic_ack(method_frame.delivery_tag)
            # else:
                # channel.basic_ack(method_frame.delivery_tag)
...
def event_log(event_info, payload, conn):
    try:
        ...
        if event_info:
            ...
            quantity_produced = payload.get('quantity_produced') if payload.get('quantity_produced') else None
            quantity_rejected = payload.get('quantity_rejected') if payload.get('quantity_rejected') else None
            ...
            cursor.execute(f''' select timezone(case when sf_plant_tzone is not null then sf_plant_tzone else 'UTC' end, timezone('UTC','{payload['data_received_time']}'::timestamp))::timestamp, 
                           timezone(case when sf_plant_tzone is not null then sf_plant_tzone else 'UTC' end, timezone('UTC','{payload['asset_previous_stime']}'))::timestamp from sf_plant where fw_tenant_id = {payload['fw_tenant_id']}
                           and sf_plant_id = {payload['sf_plant_id']}''')
            ...
            if asset_pre_status == 'up':
                ...
                if payload.get('asset_status') == 'down':
                    ...
                    if payload['start_time'].date() == payload['end_time'].date:
                        ...
                        payload['asset_runtime'] = (payload['end_time'] - payload['start_time']).total_seconds() / 60
                        eventSequence = insert_eventSequence(payload, conn)
                    payload['start_time'] = payload['tz_start_time'] if payload['tz_start_time'] else datetime.strptime(payload['data_received_time'], "%Y-%m-%d %H:%M:%S.%f")
                ...
                if payload.get('asset_status') == 'down':
                    ...
                    if payload['start_time'].date() == payload['end_time'].date:
                        ...
                        payload['fault_reason_id'] = None
                        payload['start_time'] = payload['tz_start_time'] if payload['tz_start_time'] else event_info[0]
                        payload['end_time'] = payload['start_time'].replace(hour=0, minute=0, second=0) + timedelta(
                            days=1)
                        payload['asset_downtime'] = (payload['end_time'] - payload['start_time']).total_seconds() / 60
                        eventSequence = insert_eventSequence(payload, conn)
                        payload['downtime_reason_code'] = downtime_reason_code
                        ...
                        payload['fault_reason_code'] = fault_reason_code
                        payload['fault_reason_id'] = fault_reason_id
                        payload['start_time'] = payload['tz_start_time'].replace(hour=0, minute=0,
                                                                                 second=0) + timedelta(days=1) if \
                        payload['tz_start_time'] else event_info[0]
                        payload['end_time'] = payload['tz_end_time']
                        payload['asset_downtime'] = (payload['end_time'] - payload['start_time']).total_seconds() / 60
                        eventSequence = insert_eventSequence(payload, conn)
...
def available_log(payload, conn):
    try:
        cursor = conn.cursor()
        payload['timeslot'] = int(payload.get('start_time').hour) + 1 if payload.get('start_time') else None
        print('2.1', payload['timeslot'])
        payload['previous_timeslot'] = int(payload.get('end_time').hour) + 1 if payload.get('end_time') else None
        ...
        if payload.get('asset_status') == 'down' and payload.get('asset_previous_status') == 'down':
            ...
            if payload.get('downtime_reason_id') and payload.get('previous_downtime_reason_code'):
                if existing_timeslot:
                    ...
                    availablity = availabl_event_log(payload, conn)
                    plandown_date = datetime.strptime(payload['data_received_time'], "%Y-%m-%d %H:%M:%S.%f").replace(minute=0, second=0)
                    payload['plandown_min'] = (datetime.strptime(payload['data_received_time'], "%Y-%m-%d %H:%M:%S.%f") - plandown_date).total_seconds()/60
                    availablity = availabl_event_log(payload, conn)
                    return True if availablity else False
                ...
                if existing_timeslot:
                    ...
                    availablity = availabl_event_log(payload, conn)
                    unplandown_date = datetime.strptime(payload['data_received_time'], "%Y-%m-%d %H:%M:%S.%f").replace(minute=0,second=0)
                    payload['unplandown_min'] = (datetime.strptime(payload['data_received_time'], "%Y-%m-%d %H:%M:%S.%f") - unplandown_date).total_seconds()/60
                    availablity = availabl_event_log(payload, conn)
                    return True if availablity else False
                ...
                if existing_timeslot:
                    ...
                    availablity = availabl_event_log(payload, conn)
                    plandown_date = datetime.strptime(payload['data_received_time'], "%Y-%m-%d %H:%M:%S.%f").replace(minute=0, second=0)
                    payload['unplandown_min'] = (datetime.strptime(payload['data_received_time'], "%Y-%m-%d %H:%M:%S.%f") - plandown_date).total_seconds()/60
                    availablity = availabl_event_log(payload, conn)
                    return True if availablity else False
            ...
            if existing_timeslot:
                ...
                if payload.get('quantity_produced'):
                    ...
                    if QTY_exists_timeslot:
                        ...
                        QTY = qty_event_log(payload, conn)
                lasthour= datetime.strptime(payload['data_received_time'], "%Y-%m-%d %H:%M:%S.%f").replace(minute=0, second=0)
                payload['run_min'] = (datetime.strptime(payload['data_received_time'], "%Y-%m-%d %H:%M:%S.%f") - lasthour).total_seconds() / 60
                availablity = availabl_event_log(payload,conn)
                Performance = perf_event_log(payload, conn)
            ...
            if existing_timeslot:
                ...
                cursor.execute(update)
                lasthour = datetime.strptime(payload['data_received_time'], "%Y-%m-%d %H:%M:%S.%f").replace(minute=0, second=0)
                run_time = (datetime.strptime(payload['data_received_time'], "%Y-%m-%d %H:%M:%S.%f") - lasthour).total_seconds()/60
                payload['run_min'] = run_time
                availablity = availabl_event_log(payload, conn)
                Performance = perf_event_log(payload,conn)
                if payload.get('quantity_produced'):
            ...
            if existing_timeslot:
                ...
                availablity = availabl_event_log(payload, conn)
                lasthour = datetime.strptime(payload['data_received_time'], "%Y-%m-%d %H:%M:%S.%f").replace(minute=0, second=0)
                down_time = (lasthour - datetime.strptime(payload['data_received_time'], "%Y-%m-%d %H:%M:%S.%f")).total_seconds()/60
                payload['plandown_min'] = down_time if unplanning else 0
                payload['unplandown_min'] = down_time if planning else 0
                availablity = availabl_event_log(payload, conn)
                return True if availablity else False
...
def add_dlog():
    ...
    mail_data = {'asset_status': 'up', 'cycle_time': 4, 'product_code': 'biscket', 'event_code': 777, 'fw_tenant_id': 4339, 'fw_eui_id': 'fizo125', 'fw_application_id': 1703,
                 'data_received_time': datetime.strftime(datetime.utcnow(), "%Y-%m-%d %H:%M:%S.%f"), 'sf_asset_id': 1, 'task': 'event_log', 'quantity_produced': 4}
